...
class WooIntegration
{
    ...
    public static function init()
    {
        if (fe_fs()->can_use_premium_code__premium_only()) {
            // check if user need to pay for post and id yes change post status to draft
            add_filter('bfe_ajax_before_front_editor_post_update_or_creation', [__CLASS__, 'change_post_data'], 10, 5);

            add_action('bfe_ajax_after_front_editor_post_update_or_creation', [__CLASS__, 'add_product_to_cart'], 999, 3);

            add_filter('woocommerce_get_item_data', [__CLASS__, 'display_custom_data_in_cart'], 10, 2);

            add_action('woocommerce_checkout_create_order_line_item', [__CLASS__, 'add_custom_data_to_order_items'], 10, 4);
            add_action('woocommerce_order_item_get_formatted_meta_data', [__CLASS__, 'hide_custom_data_on_thankyou'], 10, 2);

            add_action('woocommerce_thankyou', [__CLASS__, 'redirect_after_payment'], 10, 1);

            add_action('woocommerce_order_status_completed', [__CLASS__, 'update_post_status_after_payment'], 10, 1);
            add_action('woocommerce_order_status_processing', [__CLASS__, 'update_post_status_after_payment'], 10, 1);
        }
    }
    ...
    public static function add_product_to_cart($post_id, $form_id, $post_data)
    {
        ...
        $product_added = \WC()->cart->add_to_cart($product_id, 1, 0, [], $order_data);

        if (!$product_added) {
            wp_send_json_error(['message' => __('Failed to add product to cart', 'front-editor')], 500);
        }
     ...
     */
    public static function change_post_data($post_data, $post, $files, $post_id, $form_id)
    {
    ...
    public static function add_custom_data_to_order_items($item, $cart_item_key, $values, $order)
    {
