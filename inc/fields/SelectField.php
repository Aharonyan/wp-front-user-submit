...
class SelectField
{
    ...
    public static function init()
    {
        ...
        if (fe_fs()->can_use_premium_code__premium_only()) {
            add_filter('bfe_ajax_before_front_editor_post_update_or_creation', [__CLASS__, 'validate_select_field'], 10, 6);
    ...
    public static function validate_select_field($post_data, $data, $file, $post_id, $form_id, $all_settings)
    {
    ...
    public static function save_field_to_front_form($post_id)
    {
        ...
        foreach ($_POST['select'] as $name => $value) {
            if ($value['required'] === '1' && $value['ids'] === 'null') {
                /* Translators: filed is required */
                wp_send_json_error(['field' => self::$field_type, 'message' => sprintf(__('%s is required', 'front-editor'), $value['label'])]);
            }
            update_post_meta($post_id, $name, $value['ids']);
        }
    }
