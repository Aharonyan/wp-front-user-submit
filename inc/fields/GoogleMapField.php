...
class GoogleMapField
{
    ...
    public static function add_field_settings($data)
    {
        ...
        $data['formBuilder_options']['typeUserAttrs'][self::$field_type] = [
            'country_code' => ['label' => sprintf('%s', __('Country Code', 'front-editor')), 'value' => '', 'placeholder' => 'en'],
            'required_error_text' => ['label' =>  __('Required error text', 'front-editor'), 'placeholder' => 'Please add google map field', 'value' => ''],
            'not_correct_post_title_text' => ['label' => __('Not correct content', 'front-editor'), 'placeholder' => 'Please add google map field', 'value' => ''],
        ];
         ...
         */
        $data['formBuilder_options']['messages']['for_google_map_message'] = sprintf(
            __('Before using this field, please add your Google API Key in %s.', 'front-editor'),
            '<a target="_blank" href="' . esc_url(home_url('/wp-admin/admin.php?page=fe-global-settings')) . '">' . __('Settings', 'front-editor') . '</a>'
        );
