...
class PostTitleField
{
    ...
    public static function init()
    {
        ...
        // Front form template
        add_action('bfe_editor_on_front_field_adding', [__CLASS__, 'display_field_on_front_form'], 10, 3);
        ...
        add_filter('bfe_ajax_before_front_editor_post_update_or_creation', [__CLASS__, 'add_title_if_exist'], 10, 5);

        // Update title if generate title is exist
        add_action('bfe_ajax_after_front_editor_post_update_or_creation', [__CLASS__, 'check_if_generate_title'], 11, 3);
    }
    ...
    public static function validate_field_on_front_form_submit($post_id, $form_id, $all_settings)
    {
        ...
        if (!isset($settings['generate_title'])) {
            if (empty($_POST['post_title']) && $settings['required']) {
                ...
                wp_send_json_error(['field' => self::$field_type, 'message' => $message]);
            }
     ...
     * @return void
     */
    public static function add_title_if_exist($post_data, $post, $files, $post_id, $form_id)
    {
    ...
    public static function check_if_generate_title($post_id, $form_id, $post_data)
    {
        ...
        if (is_wp_error($post_id)) {
            wp_send_json_error(['field' => self::$field_type, 'message' => $post_id->get_error_message()]);
        }
    }
    ...
    public static function add_field_settings($data)
    {
        ...
        $data['formBuilder_options']['typeUserAttrs'][self::$field_type] = [
            'placeholder' => ['label' => sprintf('%s', __('Placeholder', 'front-editor')), 'value' => '', 'placeholder' => 'Add Title'],
            'title_element' => [
                'label' => 'HTML element',
                'multiple' => false,
                'options' => [
            ...
            'hide_field' => ['label' => __('Hide Field', 'front-editor'), 'value' => false, 'type' => 'checkbox'],
            'generate_title' => ['label' => sprintf('%s', __('Generate title', 'front-editor')), 'value' => '', 'placeholder' => 'New Post [taxonomy_name] [field_name]'],
            'required_error_text' => ['label' =>  __('Required error text', 'front-editor'), 'placeholder' => 'Please add post title', 'value' => ''],
            'not_correct_post_title_text' => ['label' => __('Not correct content', 'front-editor'), 'placeholder' => 'Please add correct post title', 'value' => ''],
        ];
