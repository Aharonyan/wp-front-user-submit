...
class TaxonomiesFields
{
    public static function init()
    {
        ...
        add_filter('bfe_ajax_before_front_editor_post_update_or_creation', [__CLASS__, 'validating_taxonomy_fields'], 10, 6);
        ...
        foreach ($post_taxonomies as $tax) {
            ...
            $data['formBuilder_options']['typeUserAttrs'][$tax_type] =
                [
                    ...
                    'exclude' => ['label' => __('Terms to excluded', 'front-editor'), 'placeholder' => '556,778,993', 'value' => ''],
                    'default_terms' => ['label' => __('Default terms', 'front-editor'), 'placeholder' => '128,99,73', 'value' => ''],
                    'required_error_text' => ['label' =>  __('Required error text', 'front-editor'), 'placeholder' => 'Term selection is required', 'value' => ''],
                    'search_placeholder' => ['label' => __('Search placeholder', 'front-editor'), 'placeholder' => 'Search', 'value' => ''],
                    'search_error_text' => ['label' => __('No result text', 'front-editor'), 'placeholder' => 'No Results', 'value' => ''],
                ];
            ...
            $data['formBuilder_options']['attr_descriptions']['default_terms'] = sprintf(
                __('You can specify a comma-separated term IDs that need to be included. Check the tutorial to find the term ID using this %s', 'front-editor'),
                $tutorial_link
            );
        }
     ...
     * @return void
     */
    public static function validating_taxonomy_fields($post_data, $data, $file, $post_id, $form_id, $all_settings)
    {
        ...
        foreach ($data['tax'] as $tax_name => $value) {
            ...
            if ($settings['required'] && $is_value_empty) {
                ...
                wp_send_json_error(['field' => 'tax_' . $tax_name, 'message' => $message]);
            }
        }
