...
class GoogleRecaptcha
{
    ...
    public static function init()
    {
        ...
        if (fe_fs()->can_use_premium_code__premium_only()) {
            ...
            add_action('bfe_ajax_before_post_update_or_creation', [__CLASS__, 'recaptcha_validate'], 10, 2);
        }
    }
    ...
    public static function add_field_settings($data)
    {
        ...
        $data['formBuilder_options']['typeUserAttrs'][self::$field_type] = [
            'recaptcha_language' => ['label' => sprintf('%s', __('Language', 'front-editor')), 'value' => '', 'placeholder' => 'en'],
            'recaptcha_site_key' => ['label' => sprintf('%s', __('Site key', 'front-editor')), 'value' => '', 'placeholder' => ''],
            'recaptcha_secret_key' => ['label' => sprintf('%s', __('Secret key', 'front-editor')), 'value' => '', 'placeholder' => ''],
            'recaptcha_error_message' => ['label' => sprintf('%s', __('Error Message', 'front-editor')), 'value' => '', 'placeholder' => 'Please complete the captcha properly.'],
            'recaptcha_theme' => [
                'label' => 'Theme',
                'multiple' => false,
                'options' => [
    ...
    public static function add_field_to_front_form($post_id, $attributes, $field)
    {
        ...
        printf('%s<div class="g-recaptcha" data-sitekey="%s" data-theme="%s"></div>', $script, $field['recaptcha_site_key'],$theme);
