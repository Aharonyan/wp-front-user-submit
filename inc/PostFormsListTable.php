...
class PostFormsListTable extends WP_List_Table {
    ...
    public function get_views() {
        ...
        $post_statuses  = apply_filters( 'fe_post_forms_list_table_post_statuses', [
            'all'       => __( 'All', 'front-editor'),
            'publish'   => __( 'Published', 'front-editor'),
            'trash'     => __( 'Trash', 'front-editor'),
        ] );
    ...
    public function no_items() {
        esc_html_e( 'No form found.', 'front-editor');
    }
    ...
    public function get_columns() {
        $columns = [
            ...
            'form_name'     => __( 'Form Name', 'front-editor'),
            'post_type'     => __( 'Post Type', 'front-editor'),
            'post_status'   => __( 'Post Status', 'front-editor'),
            'guest_post'    => __( 'Guest Post', 'front-editor'),
            'shortcode'     => __( 'Shortcode', 'front-editor'),
        ];
        ...
        switch ( $column_name ) {
            ...
            case 'guest_post':
                ...
                return filter_var( $is_guest_post, FILTER_VALIDATE_BOOLEAN ) ?
                            sprintf( $image, $url . 'check.svg', __( 'Yes', 'front-editor'),$style ) :
                            sprintf( $image, $url . 'cross.svg', __( 'No', 'front-editor'),$style );
    ...
    public function column_form_name( $item ) {
        ...
        if ( ( !isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) && current_user_can( fe_admin_role() ) ) {
            $actions['edit']        = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'front-editor') );
            $actions['trash']       = sprintf( '<a href="%s" class="submitdelete">%s</a>', $trash_url, __( 'Trash', 'front-editor') );
            $actions['duplicate']   = sprintf( '<a href="%s">%s</a>', $duplicate_url, __( 'Duplicate', 'front-editor') );
        ...
        if ( ( isset( $_GET['post_status'] ) && 'trash' === $_GET['post_status'] ) && current_user_can( fe_admin_role() ) ) {
            $actions['restore'] = sprintf( '<a href="%s">%s</a>', $restore_url, __( 'Restore', 'front-editor') );
            $actions['delete']  = sprintf( '<a href="%s" class="submitdelete">%s</a>', $delete_url, __( 'Delete Permanently', 'front-editor') );
        ...
        $draft_marker = ( 'draft' === $item['post_status'] ) ?
                            '<strong> — <span class="post-state">' . __( 'Draft', 'front-editor') . '</span></strong>' :
                            '';
